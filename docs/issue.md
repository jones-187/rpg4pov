...
**目标行为**：用户输入一个小场景故事设定，初始化 agent 基于该设定生成可玩的 Story Workspace。初始化结果不仅包含世界设定、基础规则、初始状态和开场主角视窗，还必须提供后续叙事智能所需的主角与角色材料。
...
Issue 6.5 提供真实 Runner、Turn History、workspace 持久化和回滚基础；Issue 7 提供 Protagonist Core、NPC voice/动机和初始冲突材料。没有这些材料，回合契约只能变成通用写作提示。
...
### 9. Adaptive Authored Protagonist：主角声音、心理描写与玩家拟合边界

**Type**: HITL
**Blocked by**: Issue 7, Issue 8
**User stories covered**: 技术架构 PRD US 87-89；产品 PRD US 106-111, 123

**目标行为**：主角拥有稳定、可识别的第一人称叙述声音。系统能自动生成符合主角人格的心理活动、低风险台词和自然反应，同时把重大关系、道德、承诺、原谅、信任和不可逆决定保留给玩家。
...
1. Protagonist Core（基础男主骨架）。
2. Confirmed Adjustments（玩家确认修正）。
3. Inferred Tendencies（推测倾向）。
4. 第一人称心理描写（Inner Monologue Guideline）。
5. 主角控制权边界（Protagonist Control Boundary）。
6. 显式纠正与持续拟合（Explicit Feedback）。
7. 基于证据和置信度的渐进拟合。
8. 低风险自动反应与重大决定的边界。
...
4. 不实现好感度数值玩法。
5. 不实现复杂情绪数值系统。
6. 不实现 Decision Point UI 或建议选项。
7. 不做性能优化。

**依赖原因**：

Issue 7 提供初始 Protagonist Core；Issue 8 让 Story Turn 能读取并使用主角人格、有效变化和角色意图。本 issue 在此基础上补充长期修正、推测倾向和控制权边界。
...
5. 玩家本回合明确输入必须覆盖基础人格和历史推测。
6. 显式长期修正高于基础人格和推测倾向。
7. 推测倾向必须包含证据和置信度。
8. 单次行为不得自动升级为稳定人格。
9. 玩家能够区分本次纠正与长期偏好。
10. 心理描写可以明确而有情绪，但关键心理结论仍交给玩家。
11. 系统可自动处理的行为和不允许自行增加的行为有明确的文档化边界。
12. 显式反馈机制支持本次纠正与长期偏好两种模式。
13. 心理描写不应长期停留在模糊中性表达，应有明确的主角声音。
...
Issue 8 需要先让回合能产生有效变化和可回应状态；Issue 9 至少提供基础主角人格与自动演出边界。否则“继续”和建议只会掩盖场景没有推进，或生成不符合主角的横向选项。
...
**Type**: HITL
**Blocked by**: Issue 5, Issue 6.5, Issue 7, Issue 8, Issue 9, Issue 10
...
8. 重大关系或不可逆决定不被系统擅自代替玩家作出。
9. 回合结尾能区分连续演出和 Decision Point。
10. 玩家仍能自由输入。
11. 随机判定仍通过 Random Tool 而不是模型假装随机。
12. 失败仍走现有 rollback 和错误日志边界。
...
**Type**: AFK
**Blocked by**: Issue 4
**User stories covered**: 技术架构 PRD US 6-7, 35-39, 61-63；产品 PRD 中 God State / NPC Memory / Random Log / NPC hiddenIntent 不对用户可见的故事

**目标行为**：系统明确验证页面只展示固定 player response。即使 agent stdout、内部日志、God State、NPC Memory、random log、Narrative Turn Contract 内部判断或 NPC hiddenIntent 中存在内容，也不会被 Web 返回给用户。固定输出缺失或格式明显不合规时，回合失败并回滚。

这个 issue 和 Issue 4 有关联，但更聚焦“主角视窗隔离”的产品风险，可以在真实 Runner 接入前后都跑。新叙事能力引入更丰富的内部角色意图后，本 issue 也要覆盖这些内部状态不外泄。

**非目标**：

1. 不实现 Narrative Turn Contract 本身。
2. 不实现 Adaptive Authored Protagonist。
3. 不实现 Decision Point UI 或建议选项。
4. 不暴露 God State、NPC Memory、random log 或内部叙事判断给普通玩家。

------

## 推荐第一轮执行范围

第一轮 MVP 路线保持从基础架构到真实可玩链路的顺序，但在原 Issue 8 之前插入叙事质量与主角控制权切片：
...
6.5. Player-visible Turn History
   ↓
7. 故事初始化：Protagonist Core + NPC voice + 初始压力源
   ↓
8. Narrative Turn Contract
   ↓
9. Adaptive Authored Protagonist
   ↓
10. Decision Points & Input Guidance
   ↓
11. 最小真实可玩链路
```

`12. 输出隔离强化` 依赖 Issue 4，可以在 Issue 6 前后或叙事 Issue 并行推进；在 Issue 11 作为完整可玩验收前应完成或至少完成覆盖新内部叙事状态的检查。
