...
### 9. Authored Protagonist Runtime：主角声音、心理描写与控制权边界
...
4. 不实现好感度数值玩法。
5. 不实现复杂情绪数值系统。
6. 不实现长期玩家行为推断（evidence/confidence 学习属于 Issue 9.5）。
...
### 9.5. Protagonist Feedback & Adaptation：显式反馈、长期偏好与推测倾向

**Type**: HITL
**Blocked by**: Issue 9
**User stories covered**: 技术架构 PRD US 88-89；产品 PRD US 108, 110-111

**目标行为**：玩家可以纠正主角表现，系统区分本次纠正与长期偏好，根据多次玩家行为逐渐形成 Inferred Tendencies，但不能由单次行为过拟合，也不能偷偷把推测升级为确定人格。
...
本 issue 属于首个可玩版本后的增强项。MVP 最小可玩链路（Issue 11）不阻塞于本 issue。如果时间有限，可先完成 Issue 9 再进入 Issue 10 和 Issue 11，待首版验证后再实现本 issue。
...
**Type**: HITL
**Blocked by**: Issue 5, Issue 6.5, Issue 7, Issue 8, Issue 9, Issue 10, Issue 12
...
Issue 9 引入了 Authored Protagonist Runtime 和主角控制权边界，内部状态包含 Protagonist Core 细节、控制权边界判定和可能的推测倾向雏形。这些内部状态不得泄漏到玩家可见输出。Issue 12 至少需要 Issue 9 完成后才能完整验证新的内部状态不外泄。
...
第一轮 MVP 路线保持从基础架构到真实可玩链路的顺序，但在原 Issue 9 处拆分为运行时和反馈两个 Issue：
...
Issue 7
Issue 8
Issue 9      ← Authored Protagonist Runtime（MVP 必需）
Issue 9.5    ← Protagonist Feedback & Adaptation（MVP 后增强项，不阻塞 Issue 11）
Issue 10
Issue 12     ← 至少依赖 Issue 9
Issue 11     ← 依赖 7/8/9/10/12
```
...
8. Narrative Turn Contract（叙事质量，不含交互状态 API）
   ↓
9. Authored Protagonist Runtime（主角运行时，不含长期拟合）
   ↓                ↘
9.5. Feedback     10. Decision Points & Input Guidance（交互控制 + Turn Interaction）
& Adaptation         ↓
(MVP 后)          12. 输出隔离强化（≥ Issue 9）
                     ↓
                  11. 最小真实可玩链路（≥ 7/8/9/10/12）
```

`12. 输出隔离强化` 至少依赖 Issue 4 和 Issue 9，可以在 Issue 10 前后或与叙事 Issue 并行推进，但必须在 Issue 11 之前完成。
`9.5. Protagonist Feedback & Adaptation` 属于 MVP 后增强项，不阻塞 Issue 10、Issue 11 或 Issue 12。
