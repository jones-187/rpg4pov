...
本计划已根据 code review 反馈修订以下 4 点：

1. **appendTurnHistory 兼容旧 story workspace**  
   - 写入前先 `mkdir path.dirname(historyPath)`，确保 `turns/` 不存在时也能创建。  
   - 补充测试：旧故事没有 `turns/history.jsonl` 时，成功回合会自动创建并 append。

2. **readTurnHistory 错误处理精确化**  
   - ENOENT/空文件返回 `[]`。  
   - `invalid storyId` 返回 `null`。  
   - JSON parse error、权限错误、其他 IO 错误应 `throw`，不静默返回空数组。  
   - 补充 malformed JSONL 测试。

3. **history append failure 的 orchestrator 测试验证 rollback**  
   - 不只断言 history 为空，还断言 `turn/output.md`、`done.json` 恢复到回合前状态。

4. **前端 story-turn 成功响应必须验证 turn 结构**  
   - 成功响应没有 `data.turn` 或结构不正确应视为响应格式错误。  
   - 验证 `turnId`、`at`、`input`、`output` 字段存在且类型正确。
...
Note: If `pnpm build` has existing environment issues, use `pnpm exec tsc --noEmit` instead for type checking.
...
# Type check (use tsc directly to avoid build environment issues)
...
- [ ] History append failure causes turn rollback (verified with file state checks)
...
- [ ] Frontend validates turn structure on successful submission
